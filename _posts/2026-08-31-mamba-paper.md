...
# Mamba: 필요한 정보만 상태에 남기는 선형 시간 Sequence Model
...
Transformer의 self-attention은 현재 token이 과거의 어느 위치를 참고할지 내용에 따라 직접 선택할 수 있다. 이 능력 덕분에 언어 모델에서 강력하지만, sequence length가 길어질수록 attention 계산량이 제곱으로 증가하고 autoregressive inference에서는 과거 전체의 KV cache를 보관해야 한다.

State Space Model(SSM)은 반대편에 있다. 과거를 고정된 크기의 hidden state로 압축하므로 sequence length에 선형으로 계산할 수 있고, 생성할 때도 길이에 비례하는 KV cache가 필요하지 않다. 그러나 기존 SSM의 state transition은 입력 내용과 무관하게 고정되어 있었다. 무엇을 기억하고 무엇을 버릴지 token마다 결정하기 어려웠고, 이것이 특히 정보 밀도가 높은 text에서 약점이 되었다.

Mamba는 이 문제를 다음 세 가지로 해결한다.

1. SSM의 $\Delta$, $B$, $C$를 현재 입력의 함수로 만들어 **선택적으로 기억하고 잊는다.**
...
> Mamba는 긴 문맥 전체를 그대로 보관하는 대신, 각 token을 읽을 때 필요한 정보만 고정 크기의 state에 선택적으로 압축하는 sequence model이다.

## 1. Attention과 Recurrent Model의 차이는 압축이다

논문은 sequence modeling의 핵심 문제를 **context compression**으로 바라본다.
...
이 차이에서 효율과 표현력의 trade-off가 생긴다.
...
Attention은 저장 비용을 지불하고 과거를 나중에 선택한다. Mamba는 **저장하는 순간 무엇을 남길지 선택**한다. 따라서 Mamba의 성능은 이 압축 정책을 얼마나 잘 학습하느냐에 달려 있다.

## 2. SSM을 먼저 이해하기

### Continuous State Space Model

State Space Model은 원래 연속 시간의 dynamic system을 표현하는 방식에서 출발한다. 입력 $x(t)$를 latent state $h(t)$에 반영하고, state에서 출력 $y(t)$를 읽는다.
...
Text는 연속 신호가 아니라 token sequence이므로 위 식을 discrete recurrence로 바꿔야 한다. Zero-order hold를 사용하면 continuous parameter $A$, $B$는 $\Delta$를 통해 $\bar A$, $\bar B$로 변환된다.
...
이 duality는 SSM의 큰 장점이지만, parameter가 시간에 따라 변하지 않아야 한다는 제약도 만든다.

## 3. 기존 LTI SSM은 왜 Text에서 약했는가
...
Static convolution은 위치별로 같은 kernel을 적용한다. 현재 token이 중요한 값인지 noise인지에 따라 state update rule을 바꿀 수 없다. Induction Heads처럼 앞에서 등장한 key-value pattern을 context에 따라 찾아야 하는 associative recall에서도 같은 문제가 나타난다.

결국 고정 크기 state 자체가 문제라기보다 **무엇을 state에 넣고 뺄지 입력에 맞춰 결정하지 못하는 것**이 문제다.

## 4. 핵심 아이디어: Selective State Space Model

Mamba의 선택적 SSM, 논문 표기로는 S6는 $A$를 제외한 $\Delta$, $B$, $C$를 현재 입력 $x_t$의 함수로 만든다.
...
세 parameter의 역할은 다음처럼 직관적으로 볼 수 있다.
...
이 설명은 직관이며 실제 계산에서는 $\Delta_t$가 discretization을 통해 $\bar A_t$, $\bar B_t$를 함께 변화시킨다.

### $\Delta$는 왜 Gate처럼 동작하는가

논문은 단순한 조건 $N=1$, $A=-1$, $B=1$에서 selective SSM이 다음 식으로 정리된다는 것을 보인다.
...
## 5. Selection을 넣자 Convolution을 잃었다
...
표현력을 얻은 대신 recurrence를 계산해야 한다. Naive implementation은 두 가지 이유로 느리다.

1. $h_t$가 $h_{t-1}$에 의존하므로 순차적으로 보인다.
2. Batch $B$, length $L$, channel $D$, state dimension $N$에 대해 확장 state의 크기가 $O(BLDN)$이 된다.

특히 $(B,L,D,N)$ 크기의 $\bar A$, $\bar B$와 중간 state를 GPU HBM에 써두고 다시 읽으면 계산보다 memory I/O가 병목이 된다. Mamba의 두 번째 기여는 이 recurrence를 GPU에서 실제로 빠르게 실행한 것이다.

## 6. Hardware-aware Selective Scan

### Parallel Scan

Recurrence는 표면적으로 순차적이지만 각 step을 associative operator로 표현하면 prefix scan으로 병렬화할 수 있다. 단순화를 위해 한 step을 다음 pair로 보자.
...
이 합성은 associative하므로 tree 형태의 parallel scan을 적용할 수 있다. Selection 때문에 linear하지는 않더라도 scan 자체는 병렬화할 수 있는 것이다.

### Kernel Fusion과 GPU Memory Hierarchy

논문의 구현에서 더 중요한 것은 계산량보다 memory movement다.
...
Mamba는 확장된 state를 느리고 큰 HBM에 materialize하지 않는다. Parameter를 HBM에서 빠른 on-chip SRAM으로 불러온 뒤 discretization과 scan을 하나의 fused kernel 안에서 처리하고 최종 output만 HBM에 쓴다.
...
## 7. Mamba Block은 어떻게 생겼는가

Selective SSM은 하나의 sequence transformation layer다. 저자들은 이를 기존 H3 block과 gated MLP에서 가져온 구조에 결합해 Mamba block을 만든다.
...
Main branch의 local convolution은 가까운 token 사이의 pattern을 먼저 섞고, selective SSM이 긴 sequence 방향의 정보를 처리한다. 다른 branch는 SiLU를 통과한 gate가 되어 main branch의 출력을 조절한다. 마지막 output projection을 거쳐 residual connection에 더한다.
...
논문 당시의 Mamba architecture에는 attention layer가 없으며 별도의 MLP block도 없다. 다만 linear projection, local convolution, activation, normalization, residual connection은 그대로 사용한다. “Attention-free”가 곧 “행렬 곱셈이 없는 모델”이라는 뜻은 아니다.
...
Mamba가 생성 step마다 유지하는 state는 sequence length와 무관하다. 따라서 context가 길어져도 KV cache가 계속 자라지 않고, 한 token을 추가하는 recurrence 비용도 과거 길이에 따라 증가하지 않는다.

하지만 이 장점에는 반대급부가 있다. Transformer는 필요할 때 과거 token을 KV cache에서 다시 직접 읽을 수 있지만 Mamba는 이미 압축된 state만 가진다. 과거의 세부 정보가 state에서 사라졌다면 원본 token으로 돌아가 복구할 수 없다. Selectivity가 중요한 이유가 바로 여기에 있다.

## 9. 실험 결과

### Selective Copying과 Induction Heads

Selective Copying ablation에서 Mamba architecture에 기존 S4 또는 Hyena layer를 넣으면 정확도가 각각 56.4%, 28.4%였다. Selective SSM인 S6를 넣은 완전한 Mamba는 99.8%를 기록했다.

Induction Heads task에서는 길이 256으로 학습한 Mamba가 1,048,576 길이까지 정확하게 extrapolation했다. 논문에서 다른 비교 방법은 학습 길이의 2배를 넘어서면서 성능이 무너졌다. 이 synthetic result는 “architecture 모양”보다 입력에 따라 state를 선택하는 mechanism이 핵심이라는 근거다.
...
Mamba는 Hyena, RWKV, RetNet, H3++ 같은 subquadratic architecture보다 좋은 scaling curve를 보였다. 특히 sequence length 8192에서 강한 Transformer++와 동등하거나 더 좋은 perplexity를 보이며, 기존 linear-time model이 언어에서 Transformer 품질을 따라가지 못한다는 격차를 줄였다.
...
Mamba-2.8B의 zero-shot 평균 63.3은 같은 표의 Pythia-2.8B 59.1뿐 아니라 Pythia-6.9B 61.7보다 높았다. 저자들이 “Mamba-3B가 약 두 배 크기의 Transformer와 경쟁한다”고 요약한 근거다.

다만 이 비교는 동일 tokenizer와 완전히 동일한 architecture를 통제한 단일 실험만은 아니다. 표에도 GPT-2, NeoX, OPT tokenizer가 섞여 있다. 따라서 수치는 당시 baseline과 training recipe 안에서 해석해야 한다.
...
이 결과에서 중요한 것은 single-request latency만이 아니다. Mamba는 KV cache가 없어 같은 GPU memory에 더 큰 batch를 넣을 수 있고, 그 결과 serving throughput이 크게 올라간다. 반대로 작은 batch, 짧은 sequence, 다른 GPU와 kernel 환경에서는 같은 배수가 보장되지 않는다.
...
## 10. Ablation에서 확인되는 진짜 핵심

논문의 ablation은 Mamba의 개선이 어디에서 오는지 비교적 명확하게 보여준다.
...
Mamba block에 non-selective S4를 넣는 것만으로는 성능이 좋아지지 않았다. H3 architecture라도 selective S6를 넣으면 크게 개선된다. 단순한 block 재배치보다 selection mechanism이 주된 기여임을 보여준다.
...
$\Delta$ 하나만 selective하게 만들어도 가장 큰 단일 개선이 생긴다. 하지만 $\Delta$, $B$, $C$를 모두 입력 의존적으로 만들었을 때 추가적인 synergy가 나타난다. “언제 갱신할지”, “무엇을 쓸지”, “무엇을 읽을지”가 함께 움직여야 한다는 뜻이다.

또한 $B$, $C$가 selective할 때는 state dimension $N$을 1에서 16으로 키우며 perplexity가 9.73에서 8.71로 크게 개선됐다. 반대로 $B$, $C$가 고정이면 같은 확장이 9.88에서 9.81로 거의 효과가 없었다. State 용량만 늘리는 것보다 그 공간에 정보를 선택적으로 넣고 읽는 능력이 중요하다.

## 11. Mamba를 이해할 때 주의할 점

### 긴 Context를 손실 없이 저장하는 것은 아니다

Mamba의 state 크기는 sequence length에 따라 늘지 않는다. 효율 면에서는 장점이지만 무한한 과거의 모든 세부 정보를 그대로 보존할 수 있다는 뜻은 아니다. 긴 context를 더 잘 활용했다는 실험 결과는 **학습된 압축이 task에 필요한 정보를 잘 남겼다**는 의미다.
...
저자들은 이를 continuous-discrete spectrum의 **no free lunch**라고 설명한다. 고정된 dynamics를 가진 LTI SSM은 audio 같은 연속 신호에 잘 맞는 inductive bias가 있다. Input-dependent selection은 text와 DNA 같은 discrete data의 약점을 해결하지만, 기존 LTI bias가 유리한 문제에서는 오히려 방해가 될 수 있다.

### 논문의 최대 Language Model은 오늘날 기준으로 작다

핵심 language scaling experiment는 약 1.3B까지, 공개된 큰 Mamba 결과는 약 2.8B 규모다. 논문도 더 큰 scale, instruction tuning, RLHF, quantization, in-context learning 같은 Transformer 생태계의 특성이 그대로 유지되는지를 향후 과제로 남긴다. 따라서 이 논문이 보여준 것은 모든 대형 Transformer를 즉시 대체했다는 결론이 아니라, **linear-time recurrent model도 language에서 Transformer 수준의 scaling 가능성을 보였다**는 것이다.

## 12. 정리

Mamba 1의 흐름을 다시 연결하면 다음과 같다.
...
Mamba의 가장 중요한 기여는 단순히 attention을 제거한 것이 아니다. 기존 SSM이 효율을 얻기 위해 포기했던 input-dependent dynamics를 되찾고, 그 때문에 생긴 hardware 문제까지 하나의 설계로 해결한 것이다.

Transformer는 과거를 넓게 보존한 뒤 query 시점에 필요한 정보를 찾는다. Mamba는 입력이 들어오는 시점에 무엇을 남길지 결정해 고정 크기의 state로 압축한다.

> Attention의 질문이 “과거 어디를 볼 것인가?”라면, Mamba의 질문은 “지금 들어온 정보 중 무엇을 미래에 남길 것인가?”다.

이 관점이 Mamba 1을 이해하는 가장 좋은 출발점이라고 생각한다.
